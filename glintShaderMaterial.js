...
function toColor3(BABYLON, v, fallback) {
  const a = Array.isArray(v) && v.length >= 3 ? v : fallback;
  return new BABYLON.Color3(Number(a[0]), Number(a[1]), Number(a[2]));
}

function toVector3(BABYLON, v, fallback, normalize = false) {
  const a = Array.isArray(v) && v.length >= 3 ? v : fallback;
  const vec = new BABYLON.Vector3(Number(a[0]), Number(a[1]), Number(a[2]));
...
export function installCenteredOctaveGaussianGlintShader(BABYLON, opts = {}) {
  ...
  BABYLON.Effect.ShadersStore[shaderName + "VertexShader"] = vertexSource;
  BABYLON.Effect.ShadersStore[shaderName + "FragmentShader"] = fragmentSource;
...
export function createCenteredOctaveGaussianGlintMaterial({
  ...
  if (!BABYLON) throw new Error("BABYLON is required.");
  if (!scene) throw new Error("scene is required.");

  const cfg = createCenteredOctaveGaussianGlintConfig(config);
  const installedName = installCenteredOctaveGaussianGlintShader(BABYLON, {
...
export function applyCenteredOctaveGaussianGlintUniforms({
  ...
  material.setVector3(
    "uLightDir",
    toVector3(BABYLON, cfg.lighting.direction, [1, 1, 1], true),
  );

  material.setColor3(
    "uFrontLightColor",
    toColor3(BABYLON, cfg.lighting.front.color, [0.95, 0.68, 0.48]),
  );
  material.setFloat("uFrontLightIntensity", Number(cfg.lighting.front.intensity));

  material.setColor3(
    "uBackLightColor",
    toColor3(BABYLON, cfg.lighting.back.color, [0.12, 0.16, 0.24]),
  );
